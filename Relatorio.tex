...
\begin{table}[h!]
  ...
  \begin{tabular}{p{3.0cm}p{3.2cm}p{3.8cm}p{4.2cm}}
    ...
    Criação do arquivo \texttt{test\_app.py} com 8 testes de regressão automatizados cobrindo todo o CRUD. \\
    \addlinespace
...
permaneça intacto. Foram contemplados 8 casos de teste:
...
\begin{itemize}[nosep]
  ...
  \item \texttt{test\_05\_edit\_user\_not\_found}: Confirma retorno HTTP 404 para ID inexistente na edição.
  \item \texttt{test\_06\_delete\_user\_success}: Confirma remoção física do usuário do banco de dados.
  \item \texttt{test\_07\_delete\_user\_not\_found}: Confirma retorno HTTP 404 para ID inexistente na exclusão.
  \item \texttt{test\_08\_full\_regression\_crud\_cycle}: Executa o ciclo completo de ponta a ponta (Create $\rightarrow$ Read $\rightarrow$ Update $\rightarrow$ Delete).
\end{itemize}
